...
| `goods` | 商品、规格和 SKU | 4 | `ADMINWRITE` | 已部署 |
...
| `image` | `string` | 是 | 商品主图的云文件 ID |
| `images` | `string[]` | 否 | 商品详情图云文件 ID 列表 |
| `isrecommend` | `boolean` | 否 | 是否在首页推荐；缺省按 `false` 处理 |
| `sort` | `number` | 否 | 同一列表内的排序值；排序方向尚未统一 |
| `status` | `string` | 是 | 当前使用字符串状态值，已出现 `"1"`；其他值含义待确认 |
| `type` | `string` | 是 | 当前使用字符串类型值，已出现 `"1"`、`"2"`；业务含义待确认 |
...
当前代码按 `categoryId` 和 `isrecommend` 查询，但云端尚无对应业务索引。商品量增加前建议评估以下索引，创建后再将其标记为“已部署”：

- `categoryId + sort`：分类商品列表。
- `isrecommend + sort`：首页推荐商品列表。
...
以下内容是目标商品与 SKU 规划，用于后续录入，不代表云端当前已有相同数量的记录。
